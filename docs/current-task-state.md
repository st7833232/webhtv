...
**下一步（唯一）**：使用者用 SideStore 更新到 `0.1.19 (20)`，先依 `docs/IOS-POC-17I-mpv-resize-libmpv.md` 第六節第 4～6 條驗 MPV 旋轉（播放中、暫停中）與子母畫面等既有功能；搜尋在 `0.1.18 (19)` 已確認不再閃退。再依 `docs/IOS-POC-20-aggregate-search.md` 的「驗收標準（更新）」實測搜尋分頁，再在真機依 17H 文件第四節驗 MPV PiP（第六節之三的真機未驗證項目），並確認 `0.1.15 (16)` 的 MPV 螢幕常亮與 `0.1.16 (17)` 的內嵌音軌／字幕切換（兩者都沒有發布前驗證紀錄）。同時使用者繼續在 `0.1.10 (11)` 上依 `docs/IOS-POC-8L-core-real-device-acceptance.md` 7.2 回報，
...
**IOS-POC-23**「暫停後離開 App 再回來，兩個核心都卡住」（2026-09-25）：只完成研究與方案，未改程式；等待使用者核准第一階段（回前景時以同一核心在原位置以暫停狀態重新載入，並在按播放前重新啟用音訊工作階段），以及回報「按播放後等 2 分鐘會不會恢復」。細節見 `docs/IOS-POC-23-pause-background-resume-stall.md`。
...
| 23 | **Research and plan only (2026-09-25), awaiting the user's approval; no code changed:** pausing, leaving the app (Home or lock, under 1 min) and returning leaves both AVPlayer and MPV black and silent, play does nothing and seeking does not recover; only reopening the player does. Diagnosis: a paused audio app is suspended, its player connections go dead or deaf and the system deactivates its audio session; MPV seeks queue behind a blocking read (mpv `demux.c`), AVPlayer items wedge (field reports), and nothing in the app reloads. Proposed stage 1: reload the current target paused on the same engine when resuming after a suspension (`appWasSuspended` or ≥ 30 s), and reactivate the audio session on the next play | `docs/IOS-POC-23-pause-background-resume-stall.md` |
