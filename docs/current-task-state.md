...
**IOS-POC-17I**（MPV 旋轉根治：自建含 resize 修正的 Libmpv）：使用者核准方案 E 與 17I-1（2026-09-25）。17I-1 的 `.github/workflows/ios-libmpv-build.yml`、`third_party/mpv-ios/`（patch、README、notice）、`third_party/mpv-ios-lock.json` 已 push，等 CI 建置、比對與發布 prerelease `mpvkit-1.0.0-webhtv.1`；App 尚未修改（17I-2、17I-3 各需另外核准）。細節見 `docs/IOS-POC-17I-mpv-resize-libmpv.md` 第十二節。子母畫面解除時放大／進度往回已診斷，依使用者決定等有模擬器再修（17H 文件）。
...
| 17I | **17I-1 in progress (2026-09-25, approved):** root fix for the MPV rotation glitch the user reported on `0.1.18 (19)` — rebuild only Libmpv from the MPVKit 1.0.0 recipe with an improved edde746/MPVKit e6b129f moltenvk resize patch, keep every other binary upstream, then drop the 17G vo rebuild. 17I-1 (build workflow, patches, lock, notices) is pushed and waits on the `iOS libmpv Build` run; 17I-2 (app switch) and 17I-3 (release) each need approval. The PiP-return zoom/rewind report is recorded in the 17H doc and deferred until a simulator is available (user decision) | `docs/IOS-POC-17I-mpv-resize-libmpv.md` |
