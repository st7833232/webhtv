...
**7.2 的全部項目在這一版或之後的版本上測（當時最新 `0.1.9 (10)`＝`0.1.8 (9)`＋來源識別修正；2026-09-25 更正：目前最新是 `0.1.20 (21)`，見 `docs/IOS-POC-11-sidestore-release.md`）**；release notes 以下方草稿為準發布。
...
建議全部在 `0.1.8 (9)` 以後的版本上做（目前最新 `0.1.20 (21)`；2026-09-25 更正，原寫 `0.1.9 (10)`）；★ 是 `0.1.7 (8)` 時期「舊版也能測」的標記。
...
（2026-09-25 更正：MPV 子母畫面已由 IOS-POC-17H 實作（`8824c8ee`），於 `0.1.11 (12)` 發布；真機上 PiP 視窗有畫面，但使用者回報過解析度降低（`0.1.12 (13)` 已修正，清晰度未再回報）與解除 PiP 時畫面放大、進度往回（使用者決定暫不修改），見 `docs/IOS-POC-17H-mpv-picture-in-picture.md`。旋轉在 `0.1.18 (19)` 真機仍會短暫跑版，之後由 IOS-POC-17I 根治，於 `0.1.19 (20)` 發布，尚待真機驗收，見 `docs/IOS-POC-17I-mpv-resize-libmpv.md`。）
...
| MPV 的 PiP／AirPlay／字幕音軌選單 | MPV 第一階段不提供（capability 關閉），第二階段才補（2026-09-25 更正：MPV 子母畫面已實作（IOS-POC-17H，`8824c8ee`），字幕／音軌選單已實作（P10，`637d3597`）；AirPlay 仍未提供，`ios/Sources/WebHTVCore/PlaybackEngine.swift:34` 為 `airPlay: false`） |
...
- 未完成：7.2 全部真機結果（**等使用者回報**；⑪ 已 superseded，⑮–⑲ 為 IOS-POC-17 新增）；
  ...
  2026-09-24 再發 `0.1.10 (11)`：加上 IOS-POC-16B 控制列 panel、15D 緩衝契約、17F 主動切換核心，它們的真機驗收也在這一版上做。
  （2026-09-25 更正：之後一路發布到 `0.1.20 (21)`，共 21 次發布，逐版內容見 `docs/IOS-POC-11-sidestore-release.md`。7.2 仍沒有逐列回報。上面 `0.1.10 (11)` 那段之後的零星真機回報，例如 MPV 子母畫面與旋轉（見 7.2 表後的更正）、`0.1.18 (19)` 搜尋不再閃退（`docs/IOS-POC-20-aggregate-search.md`），以及 `0.1.20 (21)` 上 IOS-POC-23 T1–T15 全數通過（`docs/IOS-POC-23-pause-background-resume-stall.md` 第十二節），都尚未逐列對映進 7.2。）
- 下一個動作（唯一）：使用者在目前最新的 `0.1.20 (21)` 上依 7.2 回報（2026-09-25 更正，原寫 `0.1.10 (11)`），優先 ⑱⑲；另外請留意 16B 的面板與 17F 的自動切換。
