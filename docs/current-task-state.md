...
**最新已發布版本是 `0.1.22 (23)`**（2026-09-26，使用者授權；tag `ios-v0.1.22-b23` → `450bd061`，run `36205981537`，`source.json` `b69583b2`，IPA 25,107,276 bytes，SHA-256 `4065b6bdbcfc9f237029d6c9bd4cb6c3190ae41307e6d11a9119a0ac7e87fd3f`，下載回驗通過）。至今共發布 23 版；每一版的授權、run、tag、`source.json`、IPA 大小與 SHA-256 都記錄在 `docs/IOS-POC-11-sidestore-release.md` 的各次發布，本節不再重複。
...
- IOS-POC-25「HLS 串流中段廣告自動跳過（Android parity）」（2026-09-25，另一個 session 並行做 IOS-POC-26）：已隨 `0.1.22 (23)` 發布（Release build 編譯通過，單元測試未執行）。第一次真機回報：原生仍露出不到約 1 秒的廣告開頭，正片未察覺缺少；MPV 未察覺廣告；原因未定，待使用者回報免建置檢查（IOS-POC-25 文件第二十節之二）。Android 的 `HlsAdsParser`／`HlsAdTimeline`／`resolveAdTimeline` 逐條移植成 Swift，AVPlayer 與 MPV 共用同一份計畫，以既有位置讀值與 `seek` 跳過；iOS 另外讀 playlist，所以加了讀兩次一致、duration 比對、比例上限、落點驗證等只會少跳的保護。MPV 在有 `#EXT-X-DISCONTINUITY` 的 playlist 上不跳（iOS 的 FFmpeg n8.1.2 沒有 Android FongMi 版的時間戳對齊，與 IOS-POC-26 RC3 同一原因），native-output-boundary 本階段不做。設定頁新增「智慧去廣」（預設開）。見 `docs/IOS-POC-25-hls-midstream-ad-skip.md`（第二十節為真機待驗項目）。
- IOS-POC-26「MPV／原生切換位置不對；MPV 在有廣告的影片上 seek 回到片頭、之後要重啟 App」（2026-09-25 真機回報，另一個 session 並行做 IOS-POC-25）：26-1（交接時 AVPlayer 精確落點）已 commit（`a6652cc3`），已隨 `0.1.22 (23)` 發布（Release build 編譯通過），真機未驗證；26-2 研究完成：iOS 的 FFmpeg n8.1.2 不處理 `EXT-X-DISCONTINUITY`，Android 靠 FongMi FFmpeg `5805f936` 才正確，修法是自建 iOS FFmpeg（26-2b，替換二進位，使用者 2026-09-26 核准）：FFmpeg patch 0001～0006 已在 Linux 驗證，iOS 建置管線 `.github/workflows/ios-ffmpeg-build.yml` 已加入（只建 Libavformat，發布 prerelease `ffmpeg-n8.1.2-webhtv.1`）；App 改用它（26-2b-2）與發版尚未進行。**26-2b 完成前，MPV 在有 discontinuity 的播放清單上不可做 source-time 跳廣告**（IOS-POC-25 已照此停用）。見 `docs/IOS-POC-26-engine-switch-position.md`。
...
| Current release | **WebHTV `0.1.22 (23)`**, tag `ios-v0.1.22-b23` → `450bd061`, published 2026-09-26 at the user's instruction (run `36205981537`, `WebHTV-0.1.22-23.ipa` 25,107,276 bytes, SHA-256 `4065b6bd…`, downloaded back and verified); `source.json` first entry `0.1.22`, pushed by the workflow as `b69583b2`. It is `0.1.21 (22)` plus IOS-POC-26-1 (`a6652cc3`, exact native landing on an engine switch) and IOS-POC-25 (`7530acf9`, `3243e9e0`, HLS mid-stream ad skip). **No device result yet.** Record: `docs/IOS-POC-11-sidestore-release.md` 第二十三次發布. |
...
| IOS-POC-25 HLS mid-stream ad skip | **Shipped in `0.1.22 (23)` (Release build compiled; unit tests not run). First device report 2026-09-26: native still shows under about 1 s of the ad's start, programme not noticed missing; MPV showed no noticeable ad; cause undecided, awaiting the user's no-build checks (IOS-POC-25 doc §二十.2).** Android main's built-in HLS VOD detector (`HlsAdsParser`, which 5S had wrongly read as "no m3u8 consumer": that was true only of the config `rules`) and its MPV source-timeline skip, ported to Swift and shared by AVPlayer and MPV. MPV skips only on playlists without `#EXT-X-DISCONTINUITY` (iOS FFmpeg n8.1.2 lacks Android's timestamp mapping); no native output boundary. `docs/IOS-POC-25-hls-midstream-ad-skip.md` |
