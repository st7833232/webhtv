...
## Current recovery anchor (2026-09-16, after IOS-POC-2F)
...
- Not implemented: type-0 XML; type-3 Spider/Python/DEX; `player.preloadArtwork`, `pan.*`, `app.open*`, `net.resourceUrl` proxying, `ui.setChrome`/`restoreChrome`; and the SideStore/IPA release pipeline. IOS-POC-2F drove the inline JS resolver and all seven `player.control` actions from the page, so the playback half has no unexercised paths left. A source-specific DNS or TLS error does not prove a global iOS network bug.
...
- Exactly one next action for Claude: agree the next bounded stage with the user. Ranked candidates are device deployment (needs the user's Apple ID and hardware), type-0 XML (2 sites), and driving the remaining offline-only bridge methods from a page.
