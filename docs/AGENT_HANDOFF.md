...
## Current recovery anchor (2026-09-17, after IOS-POC-5C)
...
- Active branch: `ios-poc`, HEAD `226e826c` after IOS-POC-5B, level with `origin/ios-poc`. Check the actual Git state on resume; do not infer what has been pushed.
...
- Also implemented (IOS-POC-5A/5B): a **CatVod spider runtime** in `ios/Sources/WebHTVCore/Spider/` that reimplements the `Spider.java` text-in/text-out contract in JavaScript on JavaScriptCore — one `JSContext` and serial queue per site, one shared `CatVodHost` — driving **15 further sites through 3 ported classes** (`AppGet` 5, `XBPQ` 7, `XYQHiker` 3; the last two are rule engines that serve any future site configured for them). **It never executes Android DEX or JAR bytecode**; the decompiled Java is a specification only. Contract: `docs/IOS_SPIDER_RUNTIME_SPEC.md`. **Since IOS-POC-5D those 15 sites are listed in the app UI**: `ConfigView` lists `drivableSites(resolvedBy:)` and every content call goes through `SourceClient`, which routes a site to either `CMSClient` or a cached `SpiderSession`. The app offers 45 of 167 sources. **Listed is not working, but most of it is**: IOS-POC-5E swept all 45 listed sources through the app's own path and fetched the first bytes of every resolved stream; 5F fixed five host/engine defects; 5G added a WebView media sniffer. **36 are playable** (28 of 30 native, 8 of 15 spider); 2 resolve media that 404s; 7 are empty; none list titles without episodes any more. **Every remaining failure is provider state, none is a defect in this app.** Per-site table: `docs/IOS-POC-5E-all-source-sweep.md`; the sniffer: `docs/IOS-POC-5G-media-sniffer.md`; the 苹果CMS skin fixes: `docs/IOS-POC-5I-xbpq-listing-templates.md`. **Quote 36 of 45.**
...
- **Verified 2026-09-17 at this HEAD (IOS-POC-5C):** `swift test --package-path ios` with `WANG_MOVIE_JSON` → 57 tests, 56 pass; the single failure `reportsLiveType4SitesFromProvidedConfig` is the pre-existing live-network `88看球` case (an HTML play page where the test asserts direct media) and must not be “fixed”. `xcodebuild … -scheme WebHTVApp -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -configuration Debug build` → BUILD SUCCEEDED. All three ported spiders re-verified live, each ending in `parse:0` direct media.
- Category **filter rows** (類型/地區/語言/年代/排序) exist since IOS-POC-5J, driven by CatVod's `filters` contract. Only `AppGet` sites publish them — MacCMS has no filter protocol and the rule engines do not expose one — so the rows are absent elsewhere by design. See `docs/IOS-POC-5J-category-filters.md`.
- Exactly one next action for Claude: agree the next bounded stage with the user. Candidate (1) — surfacing the ported spider sites in the app UI — was completed as IOS-POC-5D. The remaining ranked candidates are (a) batch-porting the 苹果CMS App-API family `AppQi`/`App99`/`App3Q` and then `Bili` (→ 31 sites; `AppDrama` needs RSA in `CatVodHost` first), (b) device deployment, which needs the user's Apple ID and hardware, and (c) per-request playback headers so a Referer-checking CDN can play. Full reasoning in `docs/current-task-state.md`.
