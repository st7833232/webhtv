...
**下一步（唯一）**：使用者用 SideStore 更新到 `0.1.13 (14)`，在真機依 17H 文件第四節驗 MPV PiP（第六節之三的真機未驗證項目）與 17G 旋轉。同時使用者繼續在 `0.1.10 (11)` 上依 `docs/IOS-POC-8L-core-real-device-acceptance.md` 7.2 回報，
...
| Current release | **WebHTV `0.1.13 (14)`**, tag `ios-v0.1.13-b14` → `8db58a0d`, published 2026-09-24 at the user's instruction (run `35976794989`, `WebHTV-0.1.13-14.ipa` 24,880,231 bytes, SHA-256 `41ab26d0…`, downloaded back and verified); `source.json` first entry `0.1.13`, pushed by the workflow as `1635289c`. The project carries `MARKETING_VERSION = 0.1.13` / `CURRENT_PROJECT_VERSION = 14`. It is `0.1.12 (13)` (`4548bf7b`) plus IOS-POC-19 and IOS-POC-21. **No device result yet.** |
...
| 22 | **Diagnosed 2026-09-24, fix awaiting the user's choice:** AVPlayer cannot play above 2.0× when `canPlayFastForward` is false (SDK `AVPlayerItem.h`); on the simulator it discards its whole buffer at 2.5×／3× and loops wait/jump, while 2× and MPV 3× are smooth | `docs/IOS-POC-22-avplayer-high-speed.md` |
