...
## Current recovery anchor (2026-09-16, after IOS-POC-2E)
...
- Implemented: native Swift config/CMS core; SwiftUI iPhone shell with Android-like wallpaper and settings; AVPlayer plus Infuse, Fileball, SenPlayer and VidHub; **type-1 and type-4 sources, 28 of the 167 configured**; two-level category browsing and pagination; configuration from an imported file **or any HTTPS Raw URL**, with schema validation, last-known-good caching, atomic replace, last-update status, manual refresh and a retrying launch refresh; a config-relative resource resolver; and a **WebHome bridge over `WKWebView` + `WKScriptMessageHandler`** covering the network, cache, UI, navigation, information **and playback** methods. The playback half (`player.playVod`, `playVodInline`, `control`, `status`) runs on one persistent `PlaybackSession` owning a single `AVPlayer`, which is what lets a page read a live status and control playback after the player screen is closed.
- Not implemented: type-0 XML; type-3 Spider/Python/DEX; `player.preloadArtwork`, `pan.*`, `app.open*`, `net.resourceUrl` proxying, `ui.setChrome`/`restoreChrome`; and the SideStore/IPA release pipeline. Inside IOS-POC-2E the inline JS resolver is written but **never exercised** — do not report it as working. A source-specific DNS or TLS error does not prove a global iOS network bug.
...
- Detailed status, the stage index and every unverified case: `docs/current-task-state.md`. Per-stage records: `docs/IOS-POC-1E-config-persistence.md`, `docs/IOS-POC-1F-config-sources.md`, `docs/IOS-POC-2B-webhome-bridge.md`, `docs/IOS-POC-2D-webhome-bridge-ui-info.md`, `docs/IOS-POC-2E-webhome-bridge-playback.md`, `docs/IOS-POC-4A-type4-sources.md`, and `docs/IOS-PORTING-HANDOFF-2026-09-13.md` for the original architecture assessment.
- Exactly one next action for Claude: agree the next bounded stage with the user. Ranked candidates are device deployment (needs the user's Apple ID and hardware), closing out the unverified parts of the playback half (the inline JS resolver and the `prev`/`stop`/`loop`/`replay` controls), and type-0 XML (2 sites).
