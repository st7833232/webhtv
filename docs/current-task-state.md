...
**IOS-POC-23**「暫停後離開 App 再回來，兩個核心都卡住」（2026-09-25）：第一階段已實作（`440d671e`），加上 MPV snapshot 修正（`MPVEngine.swift`，使用者同意納入範圍）；未編譯、未跑測試、真機未驗證。使用者同意修正後發布 `0.1.20 (21)`；「按播放後等 2 分鐘會不會恢復」仍待回報。細節見 `docs/IOS-POC-23-pause-background-resume-stall.md` 第十一節。
