...
**IOS-POC-17I**（MPV 旋轉根治：自建含 resize 修正的 Libmpv）：**17I-1 完成**（2026-09-25）。`.github/workflows/ios-libmpv-build.yml` 從 MPVKit 1.0.0 配方只重建 `Libmpv`，與上游比對通過（使用者核准 `_wcslen` 具名例外），發布 prerelease `mpvkit-1.0.0-webhtv.1`（run `36085794289`，`Libmpv.xcframework.zip` 3,553,297 bytes，SHA-256 `e87b4f5aea783beb2a02b4d3d1aab8197f38132d435e7907fb5ae3f6277771b4`，已寫入 `third_party/mpv-ios-lock.json`）；授權全文在 `third_party/mpv-ios/licenses/`；libbluray、libudfread、uchardet 與 `Libdovi` 內 Rust 程式碼的授權缺口已於 2026-09-25 補齊（GitHub runner 取得，逐檔核對 blob id）。**17I-2 已實作**（2026-09-25，Task-Guard `IOS-POC-17I-2`）：App 改用本地 `ios/Vendor/MPVKit` package 與 WebHTV `Libmpv`，移除 17G 的 vo 重建；Swift 編譯與真機驗收要等 17I-3 發版，17I-3 需另外核准。細節與唯一下一步見 `docs/IOS-POC-17I-mpv-resize-libmpv.md` 第十三節與 Recovery anchor。子母畫面解除時放大／進度往回已診斷，依使用者決定等有模擬器再修（17H 文件）。
...
| Current release | **WebHTV `0.1.18 (19)`**, tag `ios-v0.1.18-b19` → `ccfad785`, published 2026-09-25 at the user's instruction (run `36036441567`, `WebHTV-0.1.18-19.ipa` 25,017,725 bytes, SHA-256 `8b5e203e…`, downloaded back and verified); `source.json` first entry `0.1.18`, pushed by the workflow as `86eab8bb`. The project carries `MARKETING_VERSION = 0.1.18` / `CURRENT_PROJECT_VERSION = 19`. It is `0.1.17 (18)` (`b3c19fd4`) plus the fix for the crash on sending a search (`0786a46e`: Python boots once when several spiders ask at the same time). **No device result yet.** Record: `docs/IOS-POC-11-sidestore-release.md` 第十九次發布. |
...
| 17I | **17I-2 implemented (2026-09-25, Task-Guard `IOS-POC-17I-2`):** the app uses the local `ios/Vendor/MPVKit` package with the WebHTV `Libmpv`, and the 17G vo rebuild is gone; it compiles only in the 17I-3 release build, which needs approval. **17I-1 done (2026-09-25):** prerelease `mpvkit-1.0.0-webhtv.1` (sha256 `e87b4f5a…71b4`) passed the upstream comparison with one user-approved `_wcslen` exception. Original plan: root fix for the MPV rotation glitch the user reported on `0.1.18 (19)` — rebuild only Libmpv from the MPVKit 1.0.0 recipe with an improved edde746/MPVKit e6b129f moltenvk resize patch, keep every other binary upstream, then drop the 17G vo rebuild. The PiP-return zoom/rewind report is recorded in the 17H doc and deferred until a simulator is available (user decision) | `docs/IOS-POC-17I-mpv-resize-libmpv.md` |
