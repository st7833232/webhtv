...
**測試樣本**（使用者 2026-09-26 指定）：站台「可可影視」、線路「FF线路」、《不良一族寻爱记第二季》第 15 集，廣告約在 28:30 附近。使用者決定 M1～M7 也等有 Mac 時由 agent 自己測，與第二十節之三相同；在那之前沒有真機結果。
...
- 未解：第二十一節、第二十二節之九。
- 下一步（唯一）：有 Mac 時，以第二十二節之七的測試樣本（可可影視、FF线路、《不良一族寻爱记第二季》第 15 集，約 28:30）在 `0.1.24 (25)` 上做 M1～M7 與第二十節之三的補測（先單元測試，執行前向使用者確認一次），並量測第二十二節之九第 1 項；測到正片變少時先處理該問題。沒有 Mac 之前本任務不再動程式。
