...
- IOS-POC-29「設定頁的預設播放速度」（2026-09-26 使用者要求）：研究與規劃完成，未實作，等待使用者核准。建議 O1：新片以設定頁的速度開始，同一部片內沿用播放中調整的速度（IOS-POC-14B 不變），播放中調整不改設定。見 `docs/IOS-POC-29-default-playback-speed.md`。
