...
| Current release | **WebHTV `0.1.17 (18)`**, tag `ios-v0.1.17-b18` → `b3c19fd4`, published 2026-09-25 at the user's instruction (run `36034238374`, `WebHTV-0.1.17-18.ipa` 25,017,556 bytes, SHA-256 `e9e5b5dd…`, downloaded back and verified); `source.json` first entry `0.1.17`, pushed by the workflow as `0dff1af1`. The project carries `MARKETING_VERSION = 0.1.17` / `CURRENT_PROJECT_VERSION = 18`. It is `0.1.16 (17)` (`7acb5db1`) plus IOS-POC-20 (`ee597124`), whose first compile was this release build. **Unit tests not run; no device result yet.** Record: `docs/IOS-POC-11-sidestore-release.md` 第十八次發布. |
