...
## 6. The atomic commit is part of code completion
...
- A code change is not complete until its targeted verification is recorded and its task-owned files are committed atomically. **Recovery tags are opt-in since 2026-09-16 by the repository owner's decision** (「以後commit不用tag」); do not create one unless the user asks for it in the current task.
...
- After an explicit closure request, the next tool call must perform the closure unless one concrete unknown safety fact makes that impossible. The normal sequence is at most: one minimal patch to the existing task document only when required to record already-known results, then one `task_guard.sh finish` call that commits, then report the commit. Do not run a separate guard `check` or status/diff review, because `finish` already enforces scope. If the user did ask for a tag and the verified change is already committed, create it immediately without reopening documentation.
- Treat this as a latency-critical execution target: normally complete the requested local commit within 60 seconds and start the committing command in the first possible tool round trip. A direct command failure or genuine safety blocker is the only reason to exceed this target; report and fix only that direct cause. User confirmation never authorizes skipping an actually failed required gate, but a missing optional or neighboring test is not a blocker after the user accepts the observed behavior.
- When the same message also authorizes a push, push only what that message names — a branch authorization is not tag authorization — after local closure. Do not fetch, compare, or revalidate first unless the push itself reports a conflict or rejection.
- Finish with `bash .codex/scripts/task_guard.sh finish --verified <evidence> --commit-message <message> --no-tag`. The script must stage only task-owned paths; never hand-stage unrelated dirty work. **Pass `--no-tag` by default** and omit it only when the user asked for a recovery tag for this task.
- When a tag *was* requested, omit `--no-tag` and the script creates `recovery/<task-id>/<timestamp>` itself with one local, non-interactive `git tag -a` and signing disabled. Do not insert builds, tests, searches, network calls, repeated diff reviews, or redundant tag checks between commit and tag. Treat a successful tag command as sufficient; if it fails, capture the error once, fix its direct cause, and retry only after the command materially changes.
- If verification fails, scope overlaps pre-existing dirty work, or a safe atomic commit cannot be formed, do not commit. Record durable state when needed and report the exact blocker.
- Assessment-only work does not authorize production edits. A tag, when one is requested, points only to a committed, verified state. Never push commits/tags, move a published tag, rewrite history, or publish artifacts without explicit authorization.
