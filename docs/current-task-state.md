...
| IOS-POC-16 Custom player controls | **Implemented, builds, tests pass, and not looked at once.** AVKit draws no controls; `PlayerControlBar` draws close / subtitles / audio / speed / AirPlay / ±10 s / play / scrubber-with-buffer / opening / ending, and owns its own four-second auto-hide. **Every visual claim is unverified** — the simulator runs in that session were invalid because `ios/.build/out/...` held a 2026-09-21 binary while `xcodebuild` writes to DerivedData. **Compare the installed binary before believing any simulator observation.** `docs/IOS-POC-16-custom-player-controls.md` |
