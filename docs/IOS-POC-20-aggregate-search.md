...
- 狀態：**已實作並以 `0.1.17 (18)` 發布**（2026-09-25）：CI 的 Release device build 第一次編譯即成功；單元測試未執行；**真機未驗證**。使用者的選擇與實作內容見文末「實作紀錄（2026-09-25）」，設計依據見「設計研究（AGENTS §7，2026-09-25）」；三者衝突時，以實作紀錄為準，其次是設計研究，最後才是下方原計畫。
...
- 下一步（唯一）：使用者以 SideStore 更新到 `0.1.17 (18)`，依「驗收標準（更新）」逐條實測並回報；有問題再依回報修正。
