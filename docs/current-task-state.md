...
- Completed this session: IOS-POC-4A (type-4 CatVod remote API sources), IOS-POC-4B (ATS cleartext decision), IOS-POC-4E (bounded request timeout).
...
- `WANG_MOVIE_JSON=/tmp/webhtv-recha-new.wprHof/wang-movie.json swift test --package-path ios` → 10 tests pass, including the pre-existing 5 and a type-1 request-shape regression test.
...
- Type-4 home shows only the first category, which is visible in practice: `drpyS_听友[听]` reports 43 categories and its first one is empty, so that source looks blank. Raising this needs a category picker, not a bigger timeout.
