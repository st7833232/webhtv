...
**下一步（唯一）**：使用者用 SideStore 更新到 `0.1.20 (21)`，先依 `docs/IOS-POC-23-pause-background-resume-stall.md` 第八節 T1～T15 驗「暫停後離開再回來」（T0 只能在舊版測，已不適用），再依 `docs/IOS-POC-17I-mpv-resize-libmpv.md` 第六節第 4～6 條驗 MPV 旋轉（播放中、暫停中）與子母畫面等既有功能；搜尋在 `0.1.18 (19)` 已確認不再閃退。再依 `docs/IOS-POC-20-aggregate-search.md` 的「驗收標準（更新）」實測搜尋分頁，再在真機依 17H 文件第四節驗 MPV PiP（第六節之三的真機未驗證項目），並確認 `0.1.15 (16)` 的 MPV 螢幕常亮與 `0.1.16 (17)` 的內嵌音軌／字幕切換（兩者都沒有發布前驗證紀錄）。同時使用者繼續在 `0.1.10 (11)` 上依 `docs/IOS-POC-8L-core-real-device-acceptance.md` 7.2 回報，
...
**IOS-POC-23**「暫停後離開 App 再回來，兩個核心都卡住」（2026-09-25）：第一階段已實作（`440d671e`），加上 MPV snapshot 修正（`MPVEngine.swift`，使用者同意納入範圍）；未編譯、未跑測試、真機未驗證。已以 `0.1.20 (21)` 發布（CI 第一次編譯即成功），等待真機回報。細節見 `docs/IOS-POC-23-pause-background-resume-stall.md` 第十一節。
