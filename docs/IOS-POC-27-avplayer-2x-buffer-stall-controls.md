...
範圍外（明確不做）：不縮短 20 秒逾時；項目失敗時的交接仍沿用原請求的自動播放（開播中按暫停後若項目失敗，MPV 仍會自動播放，屬既有行為）；不改 seek 的容許誤差；不加卡頓看門狗；不改 MPV。
...
| # | 步驟 | 預期 |
|---|---|---|
| T1 | 黑畫面線路用原生開啟，不碰 | 約 0.5 秒後出現轉圈；約 20 秒改用 MPV，畫面顯示原因約 4 秒；MPV 從上次位置播放 |
| T2 | 同上，轉圈時按暫停 | 按鍵顯示暫停並可按；按下後 20 秒不會自動改用 MPV 播放；再按播放後重新計 20 秒 |
