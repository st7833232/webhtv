...
- 狀態（2026-09-25）：**已實作並 commit／push 到 `ios-poc`；未編譯、未執行單元測試、真機未驗證**（細節見第十九節）。不含於 `0.1.22 (23)`，發布要等使用者另外授權。
...
2. **沒有 native-output-boundary**：兩個核心都可能露出廣告最初約 0.1 秒。
...
- 未解：第二十一節。
- 下一步（唯一）：等使用者授權發布後，依第二十節真機驗收；首次編譯由發版 workflow 完成，編譯或測試失敗時先修本任務的程式。
