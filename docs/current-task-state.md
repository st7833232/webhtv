...
- IOS-POC-27「原生播放器：部分線路黑畫面、2 倍速容易中斷、卡住時按鍵沒反應」（2026-09-26，`0.1.24 (25)` 真機回報）：研究與規劃完成，未實作，等待使用者核准 27A（載入指示、按鍵依意圖、原生開不了時顯示原因）與 27B（預讀依倍速放大，上限 120 秒）。黑畫面不是 0.1.24 的回歸：原生開不了該線路時 App 會在 20 秒後改用 MPV，但畫面沒有任何提示。見 `docs/IOS-POC-27-avplayer-2x-buffer-stall-controls.md`。
