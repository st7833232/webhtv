...
| IOS-POC-16 Custom player controls | **Planned, not started; blocked on one user decision.** 5S-2's opening/ending controls sit on the video permanently and the user rejected that. iOS can neither extend AVKit's transport bar nor observe its visibility — both are tvOS-only — so the plan is to draw the bar ourselves. `docs/IOS-POC-16-custom-player-controls.md` |
