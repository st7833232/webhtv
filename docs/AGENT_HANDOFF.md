...
- Exactly one next action for Claude: agree the next bounded stage with the user. Ranked candidates are (1) surfacing the 15 already-verified spider sites in the app UI by routing it through `drivableSites(resolvedBy:)` instead of `supportedSites`, (2) batch-porting the 苹果CMS App-API family `AppQi`/`App99`/`App3Q` and then `Bili` (→ 31 sites; `AppDrama` needs RSA in `CatVodHost` first), and (3) device deployment, which needs the user's Apple ID and hardware. Full reasoning in `docs/current-task-state.md`.
