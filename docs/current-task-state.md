...
| IOS-POC-25 HLS mid-stream ad skip | **Implemented 2026-09-25, pushed; not compiled, unit tests not run, not device-verified; not in `0.1.22 (23)`.** Android main's built-in HLS VOD detector (`HlsAdsParser`, which 5S had wrongly read as "no m3u8 consumer": that was true only of the config `rules`) and its MPV source-timeline skip, ported to Swift and shared by AVPlayer and MPV. MPV skips only on playlists without `#EXT-X-DISCONTINUITY` (iOS FFmpeg n8.1.2 lacks Android's timestamp mapping); no native output boundary. `docs/IOS-POC-25-hls-midstream-ad-skip.md` |
