...
- 狀態：**已實作並在模擬器驗證**（2026-09-24，使用者「照預設先做19」核准，Q1–Q3 皆用預設）；`4d703da3`，已於 2026-09-24 以 `0.1.13 (14)` 發布；真機未驗證。
...
- 目前：已實作、模擬器驗證完成，`0.1.13 (14)` 發布。
- 下一步（唯一）：使用者更新到 `0.1.13 (14)` 後在真機確認切換資訊源會回到各自的站台。（2026-09-25 更正：使用者已更新到 `0.1.20 (21)`（目前最新，同樣含本功能），真機確認仍未回報。）
