...
**最新已發布版本是 `0.1.24 (25)`**（2026-09-26，使用者授權；tag `ios-v0.1.24-b25` → `5da03a4a`，run `36230882448`，`source.json` `d22fead2`，IPA 25,114,911 bytes，SHA-256 `f817206adea80a41aaf388957c51b80092fd66adf0e53b2ab5f2137bb62a7fd7`，下載回驗通過）。至今共發布 25 版；每一版的授權、run、tag、`source.json`、IPA 大小與 SHA-256 都記錄在 `docs/IOS-POC-11-sidestore-release.md` 的各次發布，本節不再重複。
...
- IOS-POC-27「原生播放器：部分線路黑畫面、2 倍速容易中斷、卡住時按鍵沒反應」（2026-09-26，`0.1.24 (25)` 真機回報）：研究、規劃與第二輪查核完成，未實作，等待使用者核准 27A（載入指示、按鍵依意圖、開播檢查只算想播的時間、原生開不了時顯示原因）與 27B（預讀依倍速放大，上限 120 秒，或選 240 秒）。黑畫面不是 0.1.24 的回歸：原生開不了該線路時 App 會在 20 秒後改用 MPV，但畫面沒有任何提示。見 `docs/IOS-POC-27-avplayer-2x-buffer-stall-controls.md`。
...
| SideStore release pipeline (IOS-POC-11) | **Done** — `.github/workflows/ios-sidestore-release.yml` and `source.json` exist and have published every release since, through `0.1.24 (25)` (25 releases, 2026-09-26) |
| Current release | **WebHTV `0.1.24 (25)`**, tag `ios-v0.1.24-b25` → `5da03a4a`, published 2026-09-26 at the user's instruction (run `36230882448`, `WebHTV-0.1.24-25.ipa` 25,114,911 bytes, SHA-256 `f817206a…`, downloaded back and verified); `source.json` first entry `0.1.24`, pushed by the workflow as `d22fead2`. It is `0.1.23 (24)` plus IOS-POC-25-2 (`45be83c4`: MPV skips HLS ads on playlists with `EXT-X-DISCONTINUITY` too, 0.25 s into each range; the app links with `-u _ff_hls_timestamp_map_segment`). The release build was IOS-POC-25-2's first compile and link. **No device result yet.** Record: `docs/IOS-POC-11-sidestore-release.md` 第二十五次發布. |
