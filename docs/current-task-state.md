...
**下一步（唯一）**：使用者用 SideStore 更新到 `0.1.14 (15)`，在真機依 17H 文件第四節驗 MPV PiP（第六節之三的真機未驗證項目）與 17G 旋轉。同時使用者繼續在 `0.1.10 (11)` 上依 `docs/IOS-POC-8L-core-real-device-acceptance.md` 7.2 回報，
...
| Current release | **WebHTV `0.1.14 (15)`**, tag `ios-v0.1.14-b15` → `618d6365`, published 2026-09-24 at the user's instruction (run `35982550285`, `WebHTV-0.1.14-15.ipa` 24,881,108 bytes, SHA-256 `13ae6d00…`, downloaded back and verified); `source.json` first entry `0.1.14`, pushed by the workflow as `13ef19d4`. The project carries `MARKETING_VERSION = 0.1.14` / `CURRENT_PROJECT_VERSION = 15`. It is `0.1.13 (14)` (`8db58a0d`) plus IOS-POC-22. **No device result yet.** |
