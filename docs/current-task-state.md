...
- Completed this session: IOS-POC-4A (type-4 CatVod remote API sources) and IOS-POC-4B (ATS cleartext decision).
...
- `WANG_MOVIE_JSON=/tmp/webhtv-recha-new.wprHof/wang-movie.json swift test --package-path ios` → 9 tests pass, including the pre-existing 5 and a type-1 request-shape regression test.
...
- Live protocol run over all 6 type-4 sites resolved `爱瓜TV` and `采集集合` to direct `.m3u8`; the other four were unreachable at run time.
...
- An unreachable site blocks for roughly 60 s on the default URLSession timeout, and a type-4 home issues two sequential requests, so a dead type-4 source can hang for up to about two minutes. Pre-existing behaviour, made more visible by type-4; no timeout was introduced because that would change type-1 request behaviour.
- Type-4 home shows only the first category, and the direct-media test is a path-extension heuristic. Both are marked with `ponytail:` comments in `CMSClient.swift`.
