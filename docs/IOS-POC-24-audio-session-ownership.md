...
3. 未編譯：與 24-2 相同，第一次編譯在發版 workflow。
4. Ponytail：unavailable / skipped。

## Recovery anchor

- 目標：兩個核心共用一個由 App 擁有的音訊工作階段（不混音的 `.playback`，播放時啟用），不再被 mpv 改成混音或在換核心時停掉原生。
- 狀態（2026-09-25）：24-1 已發布 `mpvkit-1.0.0-webhtv.2`（run `36118969804`，第一次即成功）；24-2（App 改用它並擁有工作階段）與 24-3（審查修正：中斷時暫停、類別被重設時改回）已 commit，App 尚未編譯、真機未驗證。使用者已選「審查後發布 `0.1.21 (22)`」。
- 回滾錨點：24-2 之前是 `f2dc8e65`，24-1 之前是 `e0a9194c`（見第八節）。
- 下一步（唯一）：依 `docs/IOS-POC-11-sidestore-release.md` 第二十一次發布的流程發布 `0.1.21 (22)`，由發版 workflow 做第一次編譯；發布後請使用者依第七節真機驗收。
