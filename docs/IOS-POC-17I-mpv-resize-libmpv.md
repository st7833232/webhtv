...
- 既有缺口（本任務發現，不是本任務造成）：IOS-POC-9A 的 L4 要求 notice 隨 App 出貨，App 內也要有看得到的歸屬畫面；目前 repo 與 App 都沒有。見「待你決定」Q2。（2026-09-25 更正：repo 內的 notice 已由 17I-1 與其後的 `IOS-POC-17I-licences-gaps` 收錄於 `third_party/mpv-ios/licenses/`（第十二節「授權 notice」與「授權缺口補齊」）；Q2 的決定見第十節，App 內的歸屬畫面另開任務，尚未實作。）
...
- 下一步（唯一）：等使用者在 `0.1.19 (20)` 真機回報第六節第 4～6 條；旋轉仍有問題或其他功能退步時，依第十三節回滾或修正。
  （2026-09-25 更正：使用者已更新到 `0.1.20 (21)`，同樣含 17I；在這一版上回報即可。IOS-POC-23 在 `0.1.20 (21)` 的真機驗收沒有涵蓋旋轉，第 4、5 條仍未回報；第 6 條只有 T1～T3（暫停後回到前景）有 MPV 的結果，其餘項目未記錄核心，仍需逐項回報。）
