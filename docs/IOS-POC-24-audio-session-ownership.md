...
- 狀態：**使用者 2026-09-25 選定 O3（修改 Libmpv）**，實作分兩個單元（第六節之二）。24-1 已發布 `mpvkit-1.0.0-webhtv.2`（CI 第一次即成功）；24-2、24-3 已以 `0.1.21 (22)` 發布（CI 第一次編譯即成功），**真機未驗證**。第六節的 O1＋O2 是原建議，未採用。
...
### 3. 審查與修正（IOS-POC-24-3，2026-09-25）

審查方式：兩個代理分別從「能不能編譯、並行隔離對不對」與「既有行為會不會退步」兩個角度讀 `f2dc8e65` 與 `45357898`，每一項發現再由另一個代理反駁驗證。

1. 編譯與並行：沒有發現問題（檢查 12 項）。
2. 行為：確認 2 項，都已修正在 `WebHTVApp.swift`。
   - **F1（重要）**：MPV 改成不混音後，其他 App 不混音的聲音（音樂、社群影片、語音訊息）會中斷 WebHTV；來電與 Siri 本來就會中斷。系統停掉 mpv 的 RemoteIO，但沒有人暫停 mpv，畫面停住、控制列仍顯示播放中，要按兩次才恢復。原生會自己暫停（A4）。
     - 修正：`PlaybackSession` 監聽 `AVAudioSession.interruptionNotification`，`.began` 時 `engine?.pause()`。下一次按播放會先啟用工作階段，mpv 取消暫停時重新啟動 audio unit。
     - 不在中斷結束時自動繼續播放，和原生目前的行為一致。
   - **F2（次要）**：mpv 以前每建立一個音訊輸出就重設一次類別，現在只剩啟動時會設。媒體服務重設後類別會回到預設值，之後 MPV 會被靜音鍵影響。
     - 修正：`activateAudioSession()` 發現類別、模式或選項和 App 的設定不同時，先 `setCategory(.playback, mode: .moviePlayback)`，再啟用。
3. 編譯：與 24-2 一起由 `0.1.21 (22)` 的 Release device build 第一次編譯即成功。
4. Ponytail：unavailable / skipped。

### 4. 發布（2026-09-25）

以 `0.1.21 (22)` 發布：run `36121327712`，tag `ios-v0.1.21-b22` → `255f4d8e`，IPA 25,030,951 bytes，SHA-256 `877db6d8c6114ae43f43061337f997388ad9c7342dfd783e52dac8dbf37c2c15`。下載回驗通過，細節見 `docs/IOS-POC-11-sidestore-release.md` 第二十二次發布。

## Recovery anchor

- 目標：兩個核心共用一個由 App 擁有的音訊工作階段（不混音的 `.playback`，播放時啟用），不再被 mpv 改成混音或在換核心時停掉原生。
- 狀態（2026-09-25）：24-1 已發布 `mpvkit-1.0.0-webhtv.2`（run `36118969804`）；24-2 與 24-3 已以 `0.1.21 (22)` 發布（run `36121327712`，tag `ios-v0.1.21-b22` → `255f4d8e`，Release device build 第一次編譯即成功）。單元測試未執行，**真機未驗證**。
- 回滾錨點：24-2 之前是 `f2dc8e65`，24-1 之前是 `e0a9194c`（見第八節）。
- 下一步（唯一）：等使用者在 `0.1.21 (22)` 上依第七節真機驗收，逐項記錄在本文件第十節之後；有問題先診斷，改動前先寫方案並取得核准。
