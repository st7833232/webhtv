...
## Current recovery anchor (2026-09-16)
...
- Active branch: `ios-poc`, baseline HEAD `6d926ebd` before this documentation commit, with `origin/ios-poc` at `150181b1`. Check the actual Git state on resume; do not infer what has been pushed.
- Implemented: native Swift config/CMS core; SwiftUI iPhone shell with Android-like wallpaper and settings; AVPlayer plus Infuse, Fileball, SenPlayer and VidHub; **type-1 and type-4 sources, 28 of the 167 configured**; two-level category browsing and pagination; configuration from an imported file **or any HTTPS Raw URL**, with schema validation, last-known-good caching, atomic replace, last-update status, manual refresh and a retrying launch refresh; a config-relative resource resolver; and a **WebHome bridge over `WKWebView` + `WKScriptMessageHandler`** covering the network, cache, UI, navigation and information methods.
- Not implemented: type-0 XML; type-3 Spider/Python/DEX; the playback half of the WebHome contract (`player.playVod*`, `control`, `status`) and `pan.*`, `net.resourceUrl` proxying, `ui.setChrome`/`restoreChrome`; and the SideStore/IPA release pipeline. A source-specific DNS or TLS error does not prove a global iOS network bug.
- **Of the 137 type-3 sites, 132 are structurally out of reach on iOS** — 90 `csp_*` DEX/JAR and 42 Python — not merely unimplemented. Only 5 are drpy JavaScript. Do not plan the 137 as pending work.
...
- Detailed status, the stage index and every unverified case: `docs/current-task-state.md`. Per-stage records: `docs/IOS-POC-1E-config-persistence.md`, `docs/IOS-POC-1F-config-sources.md`, `docs/IOS-POC-2B-webhome-bridge.md`, `docs/IOS-POC-2D-webhome-bridge-ui-info.md`, `docs/IOS-POC-4A-type4-sources.md`, and `docs/IOS-PORTING-HANDOFF-2026-09-13.md` for the original architecture assessment.
- Exactly one next action for Claude: agree the next bounded stage with the user. Ranked candidates are device deployment (needs the user's Apple ID and hardware), the playback half of the WebHome bridge (needs a persistent playback service), and type-0 XML (2 sites).
