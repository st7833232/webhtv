...
**IOS-POC-23**「暫停後離開 App 再回來，兩個核心都卡住」（2026-09-25）：第一階段已實作並 commit，未編譯、未跑測試、真機未驗證。發版前還要做 MPV 暫停重新載入的 snapshot 修正（`MPVEngine.swift`，待使用者同意納入範圍），以及使用者回報「按播放後等 2 分鐘會不會恢復」。細節見 `docs/IOS-POC-23-pause-background-resume-stall.md` 第十一節。
...
| 23 | **Stage 1 implemented 2026-09-25 (user-approved), not compiled, tests not run, not device-verified:** a player paused in the background is reloaded paused on the same engine at its position when a background heartbeat gap shows the app was suspended (`appWasSuspended` is deprecated since iOS 16), its tracks are selected again, and the audio session is reactivated on the next play. Pending before any release: the one-line MPV snapshot fix in `MPVEngine.swift` (outside the declared scope, awaiting approval) | `docs/IOS-POC-23-pause-background-resume-stall.md` |
